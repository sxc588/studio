...
public class RedisTest2
{
	...
	public void put(ConfigDto dto)
	{
		
		jedis.hset("Config", "key3", "value");
		jedis.set(dto.getKey(), JSON.toJSONString(dto));
	}
	...
	public void testBasicString()
	{
		...
		System.out.println(jedis.keys("*"));// 返回的sname [sname, name]
	...
	public void testMap()
	{
		...
		config.put("key2", "valu2");
		jedis.hmset("Config", config);
		...
		List<String> rsmap = jedis.hmget("Config","key*");
		System.out.println(rsmap);

		// 删除map中的某个键值
		// jedis.hdel("user","pwd");
		System.out.println(jedis.hmget("Config", "key1")); // 因为删除了，所以返回的是null
		System.out.println(jedis.hlen("Config")); // 返回key为user的键中存放的值的个数1
		System.out.println(jedis.exists("Config"));// 是否存在key为user的记录 返回true
		System.out.println(jedis.hkeys("Config"));// 返回map对象中的所有key [pwd, name]
		System.out.println(jedis.hvals("Config"));// 返回map对象中的所有value [minxr,
												// password]
